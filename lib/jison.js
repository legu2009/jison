...
lrGeneratorMixin.generateMarkdown= function parser_generate (opt) {
    opt = typal.mix.call({}, this.options, opt);
    var code = "";
    // check for illegal identifier
    if (!opt.moduleName || !opt.moduleName.match(/^[A-Za-z_$][A-Za-z0-9_$]*$/)) {
...
lrGeneratorMixin.generateModule_ = function generateModule_ () {
  ...
  lrGeneratorMixin.generateMD = function() {
	  var mermaid = ["\`\`\`mermaid", 'classDiagram', 'productions --* I0'];
	  var map = {
		'$accept': "E'",
		'$end': '$'
	  };
	  var getProduction = function (item, config) {
		  if (config) {
			  var tmp = item.handle.map(k => map[k]|| k);
			  tmp.splice(config.dotPosition, 0, '.');
			  tmp.push(',');
			  tmp.push(config.follows.map(k => map[k]|| k).join('|'));
			return `${map[item.symbol]|| item.symbol} -> ${tmp.join(' ')}`
		  } else {
			return `${map[item.symbol]|| item.symbol} -> ${item.handle.map(k => map[k]|| k).join(' ')}`
		  }
	  }
	  this.productions.forEach(item => {
		mermaid.push(`productions : ${getProduction(item)}`)
	  })
	  this.states._items.forEach((state, index) => {
		  var stateName = `I${index}`;
		  for (var symbol in state.edges) {
			mermaid.push(`${stateName} --> I${state.edges[symbol]} : ${symbol}`);
		  }
		  state._items.forEach(production => {
			mermaid.push(`${stateName} : ${getProduction(production.production, production)}`)
		  })
	  })
	  mermaid.push('\`\`\`');

	  var header = ['states', ''];
	  header.push(...this.terminals);
	  header.push('');
	  ...
	  var symbol2index = {};
	  header.forEach((symbol, index) => {
		if (index === 0) return;
		symbol2index[symbol] = index;
	  });
	  header[0] = '\`states\`';
	  header[1] = '\`action\`';
	  header[this.terminals.length + 2] = '\`goto\`';
	  var table = [
		`|${header.join('|')}|`,
		`|${new Array(len).join('-|')}-|`
	  ];
	  this.table.forEach((item, index) => {
		  var tmp = new Array(len), symbol, action;
		  tmp[0] = index;
		  for (var k in item) {
			action = item[k];
			symbol = this.symbols[k];
			if (typeof action === "number") {
				tmp[symbol2index[symbol]] = action;
			} else if (action[0] === 3) {
				tmp[symbol2index[symbol]] = 'accept';
			} else if (action[0] === 1) {
				tmp[symbol2index[symbol]] = 'S' + action[1];
			} else if (action[0] === 2) {
				tmp[symbol2index[symbol]] = 'r' + action[1];
			}
		  }
		table.push(`|${tmp.join('|')}|`)
	  })
