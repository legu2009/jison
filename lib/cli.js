...
cli.main = function cliMain(opts) {
    ...
    function processInputFile () {
		...
		opts.outMdfile = opts.outfile.replace('.js', '.md');
        ...
        var result = processGrammar(raw, lex, opts);
		fs.writeFileSync(opts.outfile, result.parser);
		fs.writeFileSync(opts.outMdfile, result.markdown);
    }
    ...
    function processStdin () {
        readin(function (raw) {
            console.log(processGrammar(raw, null, opts).parser);
        });
    }
...
cli.generateParserString = function generateParserString(opts, grammar) {
    ...
    return {
		parser: generator.generate(settings),
		markdown: generator.generateMarkdown(settings)
	}
};
